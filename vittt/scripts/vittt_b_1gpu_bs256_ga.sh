#!/bin/bash
#SBATCH --job-name=vittt_b_1gpu_bs256_ga
#SBATCH --account=aip-fsanja
#SBATCH --output=output/vittt_b_1gpu_bs256_ga/%x_%j.out
#SBATCH --error=output/vittt_b_1gpu_bs256_ga/%x_%j.err
...
srun uv run python -m torch.distributed.launch \
    ...
    --output output/vittt_b_1gpu_bs256_ga \
    --batch-size 256 \
    --epochs 30 \
    --warmup-epochs 1 \
    --wandb-run-name vittt_b_1gpu_bs256_ga \
    --amp
