#!/bin/bash
#SBATCH --job-name=vittt_b_1gpu
#SBATCH --account=aip-fsanja
#SBATCH --output=output/vittt_b_1gpu/%x_%j.out
#SBATCH --error=output/vittt_b_1gpu/%x_%j.err
...
srun uv run python -m torch.distributed.launch \
    ...
    --output output/vittt_b_1gpu \
    --batch-size 64 \
    --epochs 30 \
    --warmup-epochs 1 \
    --wandb-run-name vittt_b_1gpu \
    --amp
