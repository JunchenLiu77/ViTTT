#!/bin/bash
#SBATCH --job-name=vittt_t_1gpu
#SBATCH --account=aip-fsanja
#SBATCH --output=output/vittt_t_1gpu/%x_%j.out
#SBATCH --error=output/vittt_t_1gpu/%x_%j.err
...
srun uv run python -m torch.distributed.launch \
    ...
    main.py \
    --cfg cfgs/vittt_t.yaml \
    --data-path ~/projects/aip-fsanja/shared/datasets/imagenet/ \
    --output output/vittt_t_1gpu \
    --batch-size 64 \
    --epochs 30 \
    --warmup-epochs 1 \
    --wandb-run-name vittt_t_1gpu \
    --amp
